...
def get_good_CI(tx, dot, fontcolor, edge_width, show_lifecycle):
     ...
     dot.attr("node",shape="circle",fixedsize="true", width="1", height="1", fontname="Helvetica", fontsize="10", margin="0")
     ...
     for record in tx.run(q):
        ...
        if e1_name == "Interaction":
            ...
            if record["first"]["prev_int"] != None and record["first"]["post_int"] != None and int(record["first"]["prev_int"]) < int(record["first"]["post_int"]):
                color=c3_red
            elif record["first"]["prev_int"] != None and record["first"]["post_int"] != None and int(record["first"]["prev_int"]) > int(record["first"]["post_int"]):
                color=green
            else:
                color=gray
        ...
        if record["first"]["prev_int"] != "0" and record["first"]["prev_int"] != None:
           
            inc_node="Interaction"+":"+record["first"]["prev_int"]
        ...
        if record["second"] != None and record["second"]["prev_int"] != "0" and record["second"]["prev_int"] != None:
            node_id2=str(record["second"].id)+str(1)
            inc_node2="Interaction"+":"+record["second"]["prev_int"]
            edge_label = ""
            xlabel = ""
            pen_width = str(edge_width)
            ...
            if(e2_name)== "Interaction":
                ...
                if int(record["second"]["prev_int"])<int(record["second"]["post_int"]):
                    color=c3_red
                elif int(record["second"]["prev_int"])>int(record["second"]["post_int"]):
                    color=green
                elif int(record["second"]["prev_int"]) == int(record["second"]["post_int"]):
                    color=gray
            else:
            ...
            #e2_comp=getNodeLabel_Event(record["g_en2"]["Completed"]) 
                ...
                dot.edge(str(record["first"].id), str(record["second"].id),label=edge_label,color=edge_color,penwidth=pen_width,xlabel=xlabel,fontname="Helvetica",
                         fontsize="8",fontcolor=edge_color)
        if record["third"]["post_int"] != None: 
                edge_label = ""
                xlabel = ""
                pen_width = str(edge_width)
                edge_color = c_black
                e3_name = "Interaction"+":"+record["third"]["post_int"]
...
def get_bad_CI(tx, dot, fontcolor, edge_width, show_lifecycle):
     ...
     for record in tx.run(q):
        ...
        if e1_name == "Interaction":
            ...
            if record["first"]["prev_int"] != None and record["first"]["post_int"] and int(record["first"]["prev_int"])<int(record["first"]["post_int"]):
                color=c3_red
            elif record["first"]["prev_int"] != None and record["first"]["post_int"] and int(record["first"]["prev_int"]) > int(record["first"]["post_int"]):
                color=green
            else:
                color=gray   
        else:
        ...
        if record["second"] != None and record["second"]["prev_int"] != "0" and record["second"]["prev_int"] != None:
            ...
            if record["second"] != None:
                ...
                if(e2_name)== "Interaction":
                    ...
                    if record["second"]["prev_int"]!= None and record["second"]["prev_int"]!= None and int(record["second"]["prev_int"])<int(record["second"]["post_int"]):
                        color=c3_red
                    elif record["second"]["prev_int"]!= None and record["second"]["prev_int"]!= None and int(record["second"]["prev_int"])>int(record["second"]["post_int"]):
                        color=green
                    else:
                        color=gray
                else:
...
def get_good_FirstEntity(tx, dot, fontcolor, edge_width):
      q = '''
        ...
        '''
      dot.attr("node",shape="rectangle",fixedsize="true", width="1", height="1", fontname="Helvetica", fontsize="10", margin="0")
      for record in tx.run(q):
...
file.close()
dot.render('test-output/col_int_with_dashes_test_v3.gv', view=True)
