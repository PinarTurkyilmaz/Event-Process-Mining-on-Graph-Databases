...
def Q1(CI_ID):
    ...
    if 'Incident' in list_ent and 'Change' in list_ent:
        index= list_ent.index('Incident')     
        ...
        for i in range (len(ch_list)-1):
                ...
                if first==0:
                        ...
                        prev=prev.append({'CI':CI_ID,'Change':ch_list[first], 'prev_inc':index_first},ignore_index=True)
                        post=post.append({'CI':CI_ID,'Change':ch_list[first],'post_inc':cnt_int-1},ignore_index=True)
                        #df=df.append({'CI':ch_list[first], 'post_int':cnt_int-1},ignore_index=True)
                        prev=prev.append({'CI':CI_ID,'Change':ch_list[second], 'prev_inc':cnt_int-1},ignore_index=True)
        ...
        ##                print( 'Change:',ch_list[second],"prev_int:",cnt_int-1)
                        ...
                        if cnt_int > 1:
                                ...
                                post=post.append({'CI':CI_ID,'Change':ch_list[first], 'post_inc':0},ignore_index=True)
                                prev=prev.append({'CI':CI_ID,'Change':ch_list[second], 'prev_inc':0},ignore_index=True)
                        ...
                        post=post.append({'CI':CI_ID,'Change':ch_list[first], 'post_inc':cnt_int-1},ignore_index=True)
                        prev=prev.append({'CI':CI_ID,'Change':ch_list[second], 'prev_inc':cnt_int-1},ignore_index=True)
                        post=post.append({'CI':CI_ID,'Change':ch_list[second], 'post_inc':len(ent_ID)-index_sec-1},ignore_index=True)
...
def fun_query(CI_ID):
    ...
    query="""  
                    ...
                    where en.Name='Incident' or en.Name='Change'
    ...
    if os.stat("D:\\neo4jDatabases\\database-dfe763ee-3433-417f-9dad-04c2bbaba7bd\\installation-3.5.14\\import\\results_impact.csv").st_size>18:
        ...
        shutil.move("D:\\neo4jDatabases\\database-dfe763ee-3433-417f-9dad-04c2bbaba7bd\\installation-3.5.14\\import\\results_impact.csv",
                "D:\\BPIC-Q1\\Incident_Change\\"+CI_ID+".csv")
